...
Prefers `cryptography` if available. Falls back to a ctypes wrapper over
the system OpenSSL `libcrypto` for environments where installing `cryptography`
is painful (Entware on routers, embedded boxes without a Rust toolchain).
The public surface mimics the small subset of `cryptography.hazmat.primitives.ciphers`
that this project actually uses:
    ...
    def _load_libcrypto():
        ...
        for c in candidates:
            try:
                ...
                last_err = e
        raise RuntimeError(
            "libcrypto not found; install python3-cryptography or "
